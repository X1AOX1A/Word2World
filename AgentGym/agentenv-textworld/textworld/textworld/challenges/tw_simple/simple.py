...
def make(settings: Mapping[str, str], options: Optional[GameOptions] = None) -> textworld.Game:
    ...
    # If the per-challenge data folder is missing, create it from built-ins to
    # avoid empty KB (which would miss constant types like 'P' and 'I').
    if not os.path.isdir(KB_PATH):
        try:
            from textworld.generator.data import create_data_files
            create_data_files(KB_PATH)
        except Exception:
            # Fall back to default KB (built-in) if creation fails.
