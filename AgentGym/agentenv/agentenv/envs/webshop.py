...
class WebshopEnvClient(BaseEnvClient):
    ...
    def _post(self, path: str, data: dict[str, Any]) -> dict[str, Any]:
        ...
        for attempt in range(max_retries):
            ...
            if res.status_code == 503:
                ...
                print(data)
        assert res.status_code == 200, f"Failed to post to {path} with data {data}: {res.text}"
        return res.json()
    ...
    def _get(self, path: str) -> dict[str, Any]:
        ...
        assert res.status_code == 200, f"Failed to get from {path}: {res.text}"
        return res.json()

    def _format_admissible_actions(self, available_actions: dict[str, Any]) -> str:
        clickables = available_actions.get("clickables", []) or []
        items = [f"'click[{c}]'," for c in clickables]
        if available_actions.get("has_search_bar"):
            items.insert(0, "'search[<your query>]',")
        body = "\n".join(items)
        return (
    ...
    def _append_admissible_actions(
        self, text: str, actions: dict[str, Any] | None = None
    ) -> str:
        if actions is None:
            actions = self.get_available_actions()
        return text + "\n\n" + self._format_admissible_actions(actions)

    def observe(self, include_actions: bool = True) -> str:
        """Fetch observation text. Optionally append admissible actions."""
        observation = self._get("observation")
        if include_actions:
            return self._append_admissible_actions(observation)
        return observation

    def get_available_actions(self) -> dict[str, Any]:
        """Fetch available actions for the current state.

        Returns a dict like {"has_search_bar": bool, "clickables": [str, ...]}.
        """
        return self._get("available_actions")

    def get_instruction_text(self) -> str:
        """Fetch current instruction text for the session."""
        return self._get("instruction_text")

    def step(self, action: str, include_actions: bool = True) -> StepOutput:
