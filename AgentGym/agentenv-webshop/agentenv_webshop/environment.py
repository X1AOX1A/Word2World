...
class WebshopEnvServer:
    ...
    def close(self, env_idx: int) -> bool:
        """Close and remove a specific environment instance."""
        env = self.env.get(env_idx)
        if env is None:
            print(f"Env {env_idx} not found or already closed.")
            return False
        ...
        try:
            ...
            # Log but continue cleanup to avoid leaking references.
            print(f"Error while closing env {env_idx}: {e}")

        # Remove references so the object can be garbage-collected.
        self.env.pop(env_idx, None)
        if env_idx in self.ls:
            self.ls.remove(env_idx)

        # Reset ring-buffer pointer to avoid stale indices if we hit capacity later.
        self.now = -1
