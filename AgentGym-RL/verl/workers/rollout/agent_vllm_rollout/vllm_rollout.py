    ...
    def preprocess_prompt_to_rollout_handler(self, prompts: DataProto, n: int) -> List[RolloutHandler]:
        ...
        for i, raw_prompt in enumerate(prompts.non_tensor_batch["raw_prompt"]):
            for _ in range(n):
                ...
                # breakpoint()
                alfworld = True if "household" in raw_prompt[0]["content"] else False
                ...
                handler = RolloutHandler(
                    ...
                    task_name="alfworld" if alfworld else prompts.non_tensor_batch["item_id"][i].split("_")[0],
                    item_id=int(self.convert_alfworld_item_id(prompts.non_tensor_batch["item_id"][i])
                                if alfworld else prompts.non_tensor_batch["item_id"][i].split("_")[-1]),
                    score=0,
                    done=False,
